...
`yolojsx` supports custom theme modules (`.jsx` or `.tsx` files) via the `--theme` CLI option. A custom theme module allows you to customize the application styling tokens (colors, typography, spacing, status palettes, Ant Design component tokens) while optionally supplying reusable branded React components from the same module.
...
When passing `--theme <path>` to `yolojsx`:
...
- **Presets vs. Paths**: A value is treated as a custom file path if it starts with `.`, contains `/` or `\`, has a file extension (`.jsx`, `.tsx`, `.js`, `.ts`), or is an absolute path. Otherwise, `yolojsx` treats it as a built-in theme preset name (e.g., `github-dark`, `material`).

```sh
# Theme file in CWD
yolojsx Report.jsx --theme ./amexgbt-theme.jsx

# Theme file in a subfolder
yolojsx src/Report.jsx --theme ./src/themes/brand-theme.jsx

# Theme file in a shared directory
yolojsx App.jsx --theme ../shared-themes/company.jsx
```
...
```sh
yolojsx src/reports/QuarterlyReport.jsx --theme ./themes/corporate-theme.jsx
```
...
1. **Default Export**: A valid `ThemeDefinition` object satisfying the yolojsx theme schema (colors, typography, rhythm, components, radius, shadow, controlHeight).
...
// Default export for yolojsx CLI theme engine
export default {
  id: "corporate",
  name: "Corporate",
  appearance: "light",
...
- **Top-level execution context**: Theme modules are loaded and compiled server-side during the `yolojsx` build step using Vite SSR. Do not execute browser-only DOM globals (`window`, `document`, `localStorage`) at the top level of the theme module outside of React component lifecycle/hooks.
- **Dependencies**: Custom theme modules can import React, Ant Design (`antd`), React Icons (`react-icons/*`), PrismJS, or relative local files. Unresolved third-party imports outside `yolojsx` supplied stack require presence in local `node_modules`.
- **Discovery**: Custom theme modules do not appear in `yolojsx themes` CLI output (which lists built-in presets).
