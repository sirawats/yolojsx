...
import type { EmbeddedPayload } from "../src/single-file.js";
...
function runCli(packageDirectory: string, args: string[], expectedStatus = 0) {
  return run(
    process.execPath,
    [path.join(packageDirectory, "lib/bin.js"), ...args],
    {
      cwd: workDirectory,
      expectedStatus,
    },
...
try {
  ...
  const packedMetadata: unknown = JSON.parse(packed.stdout);
  const firstEntry: unknown = Array.isArray(packedMetadata)
    ? packedMetadata[0]
    : undefined;
  if (
  ...
  await writeFile(
    path.join(workDirectory, "Home.tsx"),
    `import { Button } from "antd";
const label: string = "Package verification";
export default () => <main className="p-8"><Button>{label}</Button></main>;
`,
    "utf8",
  );
  ...
  for (const name of themeFiles) {
    const stylesheet = await readFile(
      path.join(packageDirectory, "lib/themes", name),
      "utf8",
    );
  ...
  const moduleUrl = pathToFileURL(
    path.join(packageDirectory, "lib/single-file.js"),
  );
  const { readEmbeddedPayload } = (await import(moduleUrl.href)) as {
    readEmbeddedPayload: (html: string) => EmbeddedPayload;
  };
  for (const name of [
    "Home.html",
    "index.html",
