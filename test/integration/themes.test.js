...
test("builds every fixed preset with global, Tailwind, and Ant Design styling", async (t) => {
  ...
  await writeFixture(fixture, {
    "Catalog.jsx": `import { Alert, Button } from "antd";
export default function Catalog() {
  return <main className="min-h-screen bg-yolo-canvas p-8 text-yolo-text">
    <h1>Theme catalog</h1><p className="yolo-muted">Semantic global styles</p>
    <Button type="primary">Action</Button><Alert type="success" message="Ready" />
  </main>;
}`,
  });
  ...
  for (const theme of THEMES) {
    ...
    assert.match(css, /\.bg-yolo-canvas\{/i, theme.id);
    assert.match(css, /\.text-yolo-text\{/i, theme.id);
    assert.match(css, /--yolo-heading-weight:/, theme.id);
...
:root { --yolo-primary: #7346a8; }
@layer components {
  .custom-card { background-image: url("./mark.svg"); border: 3px solid var(--yolo-primary); }
}
@utility content-auto { content-visibility: auto; }`,
  ...
  assert.ok(
    directoryCss.lastIndexOf("--yolo-primary:#7346a8") >
      directoryCss.indexOf("--yolo-primary:#6750a4"),
...
test("lists themes and resolves aliases through the CLI", async (t) => {
  ...
  assert.match(listing.stdout, /catppuccin-latte \(fixed light; aliases: catppuccin\)/);
  assert.match(listing.stdout, /aliases: onedark/);
  ...
  assert.match(lightCss, /--yolo-canvas:#fffbfe/);
