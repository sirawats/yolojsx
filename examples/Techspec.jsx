...
export default function Techspec() {
  return (
    <main className="min-h-screen bg-yolo-canvas text-yolo-text">
      <header className="border-b border-yolo-border bg-yolo-surface px-6 py-4 lg:px-10">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
          <div>
            <Typography.Text className="font-yolo-mono text-xs uppercase tracking-[0.18em] text-yolo-text-muted">
          ...
          <Space wrap>
            ...
            <Typography.Text className="text-yolo-text-muted">Updated 22 Jul 2026</Typography.Text>
          </Space>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-6 py-8 lg:grid-cols-[220px_minmax(0,1fr)] lg:px-10">
        <aside className="lg:sticky lg:top-6 lg:self-start">
          ...
          <nav className="mt-4 grid gap-1 text-sm text-yolo-text-muted">
            {["Summary", "Context", "Architecture", "Requirements", "Rollout"].map((item, index) => (
              <a key={item} href={`#${item.toLowerCase()}`} className={`rounded-md px-3 py-2 ${index === 0 ? "bg-yolo-code-background text-yolo-primary" : "hover:bg-yolo-surface"}`}>
                {String(index + 1).padStart(2, "0")} · {item}
              </a>
            ))}
          </nav>
          <Card size="small" className="yolo-surface mt-6">
            <Typography.Text className="text-xs text-yolo-text-muted">Review progress</Typography.Text>
            <Progress percent={78} size="small" className="mt-2" />
            <Typography.Text className="text-xs text-yolo-text-muted">7 of 9 reviewers</Typography.Text>
          </Card>
        </aside>
        ...
        <article className="min-w-0">
          <section id="summary" className="mb-10">
            ...
            <Typography.Paragraph className="max-w-3xl !text-lg !leading-8 text-yolo-text-muted">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div key={label} className="rounded-lg border border-yolo-border bg-yolo-surface p-4">
                  <div className="text-xs uppercase tracking-wider text-yolo-text-muted">{label}</div>
            ...
            <div className="mt-5 grid items-stretch gap-3 md:grid-cols-[1fr_auto_1fr_auto_1fr]">
              {[
                ...
                <div className="contents" key={title}>
                  <Card className="yolo-surface">
                    <span className="font-yolo-mono text-xs text-yolo-primary">{number}</span>
                    <Typography.Title level={4} className="!mb-1 !mt-3">{title}</Typography.Title>
                    <Typography.Text className="text-yolo-text-muted">{copy}</Typography.Text>
                  </Card>
                  {index < 2 && <div className="hidden self-center text-2xl text-yolo-text-muted md:block">→</div>}
                </div>
              ))}
            </div>
          </section>
          ...
          <section id="requirements" className="mb-10">
            ...
            <Table
              className="overflow-hidden rounded-lg border border-yolo-border"
              columns={columns}
          ...
          <section id="rollout">
            ...
            <Timeline
              className="mt-6"
              items={[
                { color: "green", children: <><strong>Week 1</strong><br /><span className="text-yolo-text-muted">Shadow verification in one region</span></> },
                { color: "blue", children: <><strong>Weeks 2–3</strong><br /><span className="text-yolo-text-muted">Progressive traffic ramp and revocation drills</span></> },
                { color: "gray", children: <><strong>Weeks 4–6</strong><br /><span className="text-yolo-text-muted">Global rollout, then remove synchronous lookups</span></> },
              ]}
            />
          </section>
        </article>
